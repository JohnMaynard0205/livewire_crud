                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-md-6">
                            @if($product->attachment)
                                <div class="mb-2">
                                    <img src="{{ asset('uploads/' . $product->attachment) }}" alt="Current product image" class="img-thumbnail" style="max-width: 150px; max-height: 150px;">
                                    <p class="form-text text-muted mt-1">Current image: {{ $product->attachment }}</p>
                                </div>
                            @endif
                            <input type="file" class="form-control @error('attachment') is-invalid @enderror" id="attachment" name="attachment" accept="image/*">
                            <small class="form-text text-muted">Supported formats: JPG, JPEG, PNG, GIF, WEBP (Max size: 2MB). Leave empty to keep current image.</small>
                            @error('attachment')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
